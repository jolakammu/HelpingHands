...
public class AddressManager {
	...
	public int getAddress(Address address) {
		...
		try {
				...
				ps = connection.prepareStatement("select Coalesce(ADDRESS_ID,0) as ADDRESS_ID from app.HH_ADDRESS where DELIVERY_TXT = ? and CITY = ? and STATE_CD = ? and COUNTRY_CD = ?");
	...
	public int addAddress(Address address) {
		...
		try {
				Connection connection;
				connection = ds.getConnection();
				ps = connection.prepareStatement("select Coalesce(Max(ADDRESS_ID),0) + 1 as ADDRESS_ID from app.HH_ADDRESS");
				...
				ps = connection.prepareStatement("Insert into app.HH_ADDRESS(ADDRESS_ID, DELIVERY_TXT,CITY,STATE_CD,COUNTRY_CD,ZIP_TXT) values (?,?,?,?,?,?)");
